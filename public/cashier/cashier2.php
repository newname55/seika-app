...
// ticket_id は最初に確定（store_id/business_date の優先順位に影響）
...
$stores = [];
if ($isSuper || $isAdmin) {
  $stores = $pdo->query("SELECT id,name,business_day_start FROM stores WHERE is_active=1 ORDER BY id ASC")
  ...
  if (function_exists('repo_allowed_stores')) {
    ...
    if ($stores) {
      ...
      foreach ($st->fetchAll(PDO::FETCH_ASSOC) ?: [] as $r) {
        $m[(int)$r['id']] = (string)($r['business_day_start'] ?? '');
      }
      foreach ($stores as &$s) {
        $sid = (int)($s['id'] ?? 0);
...
// ticket があれば ticket の store_id/business_date/status を最優先
$ticket_business_date = '';
$ticket_status_ui = 'open';
if ($ticket_id > 0) {
  $st = $pdo->prepare("SELECT id, store_id, business_date, status FROM tickets WHERE id=? LIMIT 1");
  ...
  $storeId = (int)($t['store_id'] ?? $storeId);
  $ticket_business_date = (string)($t['business_date'] ?? '');
  $ticket_status_ui = (string)($t['status'] ?? 'open');
}

// 許可チェック（ticket で店が変わった可能性があるので最後に確定）
...
// business_date は ticket 優先 / 無ければ店の営業日開始で算出
$business_date = ($ticket_id > 0 && $ticket_business_date !== '')
  ? $ticket_business_date
  : business_date_for_store($bizStart);

// cast候補（当日出勤）
$castCandidates = [];
try {
  // store_attendance を想定（無ければ空でOK）
  if ($pdo->query("SHOW TABLES LIKE 'store_attendance'")->fetchColumn()) {
    $st = $pdo->prepare(
      "SELECT cast_no, name
         FROM store_attendance
        WHERE store_id=? AND business_date=? AND status='present'
        ORDER BY cast_no ASC"
    );
    $st->execute([$storeId, $business_date]);
    $castCandidates = $st->fetchAll(PDO::FETCH_ASSOC) ?: [];
  }
} catch (Throwable $e) {
  $castCandidates = [];
}
...
$right = '
  ...
  <a class="btn" href="/seika-app/public/cashier/index.php?store_id='.(int)$storeId.'">新 会計一覧</a>
';
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    /* ====== Highlight blocks to reduce misclick ====== */
    .blockSafe{
      border-radius: 18px;
      padding: 12px;
      border: 1px solid var(--line);
    ...
    /* ====== Your legacy classes compatibility ====== */
    ...
    @media (max-width:1024px){
      .drinkRowTight{ grid-template-columns: 1fr 1fr; }
    }

    /* =========================
       iPad現場向け：タップ大ボタン＆詰めUI 追加
       ========================= */
    .tabXL{
      flex: 0 0 auto;
      min-height: 78px;
    ...
    .btn2{border:none;border-radius:14px;padding:12px 14px;min-height:52px;font-weight:1000;box-shadow:0 8px 14px rgba(0,0,0,.08);background:#fff;border:1px solid var(--line)}
    .drinkRow{display:grid;grid-template-columns:160px 140px 1fr 110px;gap:10px;align-items:center;margin-bottom:8px}
  ...
  <div class="stickyHeader">
    <div class="headCard">
      ...
      <div class="actionGrid">
        <a class="btn b-dark" href="/seika-app/public/dashboard.php">ダッシュボード</a>
        <a class="btn b-blue" href="/seika-app/public/cashier/index.php?store_id=<?= (int)$storeId ?>">新 会計一覧</a>
        <button class="btn b-cyan" type="button" id="freeHistBtn">FREE履歴</button>
        <button class="btn b-green" type="button" id="previewBtn">プレビュー</button>
        <button type="button" class="btn b-purple" id="serverCalcBtn_head">サーバ計算</button>
        <button class="btn b-red" type="button" id="closeBtn">精算完了（CLOSE）</button>
      </div>
        ...
        <div class="card">
          ...
          <div class="grid4">
            ...
            <div class="full" style="display:grid;grid-template-columns: repeat(4, 1fr); gap:10px;">
              ...
              <button type="submit" class="btn b-dark">保存（DB）</button>
              <button type="button" class="btn b-yellow" id="goIndex2Btn">一覧へ戻る</button>
            </div>
...
<div class="closedOverlay" id="closedOverlay">
  <div class="closedModal">
    ...
    <div style="display:flex;gap:10px;margin-top:12px;flex-wrap:wrap;">
      <a class="btn b-blue" href="/seika-app/public/cashier/index.php?store_id=<?= (int)$storeId ?>">会計一覧へ</a>
      <a class="btn b-dark" href="/seika-app/public/dashboard.php">ダッシュボードへ</a>
    </div>
  </div>
</div>
...
(function(){
  ...
  if (goIndex2) goIndex2.addEventListener('click', ()=>{
    location.href = '/seika-app/public/cashier/index.php?store_id=<?= (int)$storeId ?>';
  });
})();
</script>
...
<script>
  ...
  // CLOSEボタン（haruto_core にはまだ close API が無い想定なので UIだけ）
  (() => {
    ...
    if (!window.TICKET_ID) {
      ...
      closeBtn.textContent = '精算完了（保存後に有効）';
      closeBtn.title = '先に保存してください';
      return;
    }

    if (window.TICKET_STATUS === 'closed') {
      closeBtn.disabled = true;
      closeBtn.classList.add('disabled');
      closeBtn.textContent = '精算済み（PAID）';
      closeBtn.title = 'この伝票は会計済みです';
      return;
    }

    closeBtn.addEventListener('click', async () => {
      alert('CLOSEはまだAPI未実装です（haruto_core側で ticket_close を作ったら繋ぎます）');
    });
  })();
  ...
  async function loadCastCandidates(){
    ...
    const r = await fetch(
    `/seika-app/public/api/cast_candidates.php?store_id=${encodeURIComponent(shopId)}&business_date=${encodeURIComponent(businessDate)}`,
    { cache:'no-store' }
    );
  ...
  function setGuestPeople(setIndex, val){
    const s = state.sets[setIndex];
    const before = s.guest_people;
    s.guest_people = Math.max(0, parseInt(val || "0", 10));
    normalizeCustomers(s);
    if (before !== s.guest_people) logEvent('guest_people_change', setIndex+1, {before, after:s.guest_people});
    render(true);
  }
  ...
  function formatFreeHistory(history){
    ...
    const head = `
      <style>
        ...
        .freeGrid .setHead{background:#111;color:#fff;font-weight:900}
      ...
      return `
        <div class="freeGridWrap" style="margin:12px 0;">
          <table class="freeGrid">
            <thead>
              <tr><th class="setHead" colspan="5">セット${setNo}</th></tr>
              <tr>
  ...
  async function calcOnServer(){
    ...
    const r = await fetch('/seika-app/public/api/ticket_calc.php', {
      method:'POST',
  ...
  async function loadFromDB(ticketId){
    ...
    const r = await fetch(`/seika-app/public/api/ticket_get.php?ticket_id=${encodeURIComponent(ticketId)}`, {cache:'no-store'});
    const txt = await r.text();
  ...
  async function saveToDB(){
    ...
    const r = await fetch('/seika-app/public/api/ticket_save.php', {
      method:'POST',
      headers:{'Content-Type':'application/json'},
      credentials:'same-origin',
    ...
    setPaneEl.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
        ...
        <div>
          ...
          <span class="badge">席 ${escapeHtml(seatLabelById(s.seat_id || 0))}</span>
      ...
      <div class="drinkRowTight">
        <div>
          <label>金額（税別）</label>
          <input type="number" id="drink_amount" min="0" value="0" inputmode="numeric">
          <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">
            <button type="button" class="btnSmall b-blue" id="btnAmt1000">+1000</button>
            <button type="button" class="btnSmall b-blue" id="btnAmt1500">+1500</button>
            <button type="button" class="btnSmall b-yellow" id="btnAmtClear">クリア</button>
          </div>
        </div>
