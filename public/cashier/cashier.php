...
$ticket_status = 'open';
if ($ticket_id > 0) {
  $st = $pdo->prepare("SELECT status FROM tickets WHERE id = ? LIMIT 1");
  $st->execute([$ticket_id]);
  $ticket_status = (string)($st->fetchColumn() ?: 'open');
}
...
(() => {
  ...
  if (goIndex2) {
    goIndex2.addEventListener('click', () => {
      const url = '/seika-app/public/cashier/index.php?store_id=<?= (int)$storeId ?>';
...
<aside id="payDrawer" style="
  position:fixed; top:0; right:0; height:300vh;
...
function openDrawer(url){
  ...
  closeBtn.addEventListener('click', (e) => {
    ...
    const url =
      `/seika-app/public/cashier/payments.php`
      + `?ticket_id=${ticketId}`
      + `&store_id=${storeId}`
